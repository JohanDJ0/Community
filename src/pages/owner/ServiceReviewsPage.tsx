      ...
      <div className='second-div'>
        <div className={`box-div ${darkMode ? 'dark' : 'light'}`} style={{ position: 'relative' }}>
            ...
            <CardContent>
              <Stack spacing={2} direction="row">
                ...
                {!isFollowing && ( // Renderiza el botón solo si is_followed es false
                  <Button onClick={() => handleFollow()} variant="contained" startIcon={<AddIcon />} style={{fontSize: isSmallScreen ? '0.7rem' : '0.9rem',padding: isSmallScreen ? '4px 8px' : '6px 12px',}}>{t("Follow")}</Button>
                )}
                {isFollowing && (
                  <Typography
                    variant="body1"
          ...
          <Dialog open={openModal} onClose={handleCloseModal}>
            <DialogTitle>{t("CreateReviews")}</DialogTitle>
            <DialogContent>
